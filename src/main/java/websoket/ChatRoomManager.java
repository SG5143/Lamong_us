...
public class ChatRoomManager {
    ...
    public boolean addClientToRoom(String roomType, String roomCode, Session session) {
        roomClients.computeIfAbsent(roomType, k -> new HashMap<>())
                   .computeIfAbsent(roomCode, k -> new HashSet<>());
    ...
    public List<ChatRequestDto> getRoomChatHistory(String roomType, String roomCode) {
        return roomChatHistory.getOrDefault(roomType, Collections.emptyMap())
                              .getOrDefault(roomCode, Collections.emptyList());
    }

    public Set<Session> getClientsInRoom(String roomType, String roomCode) {
        return roomClients.getOrDefault(roomType, Collections.emptyMap())
                          .getOrDefault(roomCode, Collections.emptySet());
    }
}